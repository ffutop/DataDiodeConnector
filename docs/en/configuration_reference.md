...
| `bipBufferElementCount` | `2` | Size of the internal ring buffer (unit: number of elements, approx 1MB per element). |
| `statsServerAddress` | `"0.0.0.0"` | Address of the StatsD statistics server. |
| `statsServerPort` | `8081` | Port of the StatsD statistics server. |
...
| `sendDelayMs` | `5` | **Flow Control Delay**: Wait time (milliseconds) after sending each UDP packet. Used to prevent overwhelming the physical diode or receiver. |

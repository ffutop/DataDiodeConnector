---
title: 网闸连接器部署拓扑与高可用方案 (HA)
description: 详解 DDC 在单向网络中的部署模式。涵盖单链路、多链路并行、Kafka 消费者组故障转移 (Active-Active) 及冷备方案，确保 1:1 严格配对原则。
head:
  - - meta
    - name: keywords
      content: 网闸部署, 高可用架构, HA方案, Kafka故障转移, 链路冗余, 单向网络拓扑, Active-Active
seo:
...
在企业级环境中，部署网闸连接器时必须遵循**严格的一对一绑定原则**。本文档概述了基于此核心约束的架构模式与**高可用 (High Availability, HA)** 策略。
...
### 原理说明
入口代理与出口代理内部维护着针对单一数据流的状态机（包括[序列号追踪](/zh-CN/protocol#头部字段)、分片重组状态）。如果多个入口代理向同一个出口代理发送数据，会导致接收端的序列号冲突和状态混乱，进而导致严重的数据包丢失和重组失败。
...
- **配置**：
    - [入口代理](/zh-CN/configuration_reference#入口代理配置)配置唯一的目标 UDP 地址/端口。
    - [出口代理](/zh-CN/configuration_reference#出口代理配置)监听对应的 UDP 端口。
...
- **前置条件**：
    - 每个链路对的入口代理必须作为 [Kafka Consumer](/zh-CN/configuration_reference#kafka-模式-protocolhandler-kafka) 运行。

---
title: 软件架构设计与原理 - 网闸连接器 (DDC)
description: 深入解析网闸连接器基于 Rust 的高性能软件架构。了解入口代理、出口代理、无锁流水线设计 (Lock-free Pipeline) 以及如何实现安全的单向数据传输。
head:
  - - meta
    - name: keywords
      content: 网闸软件架构, Rust 网络编程, 单向光闸, UDP 可靠传输, 无锁队列, BipBuffer, 高并发数据传输
seo:
...
**网闸连接器 (Data Diode Connector)** 是一款专为**高安全单向网络环境**设计的软件套件。它采用模块化架构，基于 **Rust** 语言构建，旨在解决物理隔离 (Air Gap) 环境下的**高性能、高可靠数据传输**难题。本文档概述了驱动该系统的软件架构、组件交互和数据流原则。

## 架构概览

### 逻辑架构拓扑

下图展示了网闸连接器的核心逻辑拓扑：数据从**高安全区 (High Side)** 的入口代理流出，经过物理单向网闸，单向流入**低安全区 (Low Side)** 的出口代理。
...
1. **入口代理 (Ingress Proxy / 发送端)**：部署在高安全网络（源端）。它从源系统（Kafka, UDP 等）消费数据，可选择性地进行[安全过滤](/zh-CN/security_model)，并将数据推送到单向网闸。
...
- **功能**：内容检查、验证、清洗或阻断。
- **行为**：如果数据包违反[安全过滤规则](/zh-CN/security_model)（例如包含 "SECRET" 关键字），它将立即被丢弃并记录日志。
- **链式调用**：多个过滤器可以串联在一起，形成一个过滤链。

### 3. 传输处理器 (Transport Handler)

**传输处理器**管理跨越物理隔离/网闸的实际传输。
- **入口端 (`transport-udp-send`)**：从缓冲区获取数据，将其分片为 UDP 数据包，添加序列号头，并将其发送到网闸接口。它实现了[流量控制机制](/zh-CN/flow_control) (`send_delay_ms`) 以防止淹没硬件设备。
- **出口端 (`transport-udp-receive`)**：在网闸接口上监听，重新排序数据包，检查缺漏（序列号），重组分片，并将完整的载荷写入下一个缓冲区。有关协议细节，请参阅 [UDP 传输协议规范](/zh-CN/protocol)。
...
3. **可配置的延迟 vs 吞吐量**：通过 `bip_buffer_element_count` 和 `send_delay_ms`，系统管理人员可以针对低延迟（小缓冲区，低延迟）或高吞吐量（大缓冲区，批量发送）[对系统进行调优](/zh-CN/flow_control)。